...
app.post("/api/installation/test", requireAdmin, async (req, res) => {
  const started = Date.now();
  try {
    const submitted = req.body ?? {};
    if (submitted.companyName || submitted.siigoUsername || submitted.partnerId || submitted.accessKey) {
      const companyName = String(submitted.companyName ?? "").trim();
      const siigoUsername = String(submitted.siigoUsername ?? "").trim();
      const partnerId = String(submitted.partnerId ?? "").trim();
      if (!companyName || !siigoUsername || !partnerId) return res.status(400).json({ error: "Empresa, usuario Siigo y Partner-ID son obligatorios" });
      store.configure({ companyName, siigoUsername, partnerId, accessKey: submitted.accessKey ? String(submitted.accessKey) : undefined });
    }
    const result = await new SiigoClient(store.credentials(), process.env.DEMO_MODE === "true").request<unknown>("/products?page=1&page_size=1");
    store.setConnectionStatus("connected");
    store.audit("connection.test", "success", "Conexión con Siigo validada", Date.now() - started);
    res.json({ ok: true, installation: store.publicInstallation(), sample: result });
  } catch (error) {
    store.setConnectionStatus("error");
    const message = error instanceof Error ? error.message : "No fue posible conectar";
    store.audit("connection.test", "failed", message, Date.now() - started);
    const status = (error as { status?: number }).status === 401 ? 401 : 502;
    res.status(status).json({ error: message, code: (error as { code?: string }).code ?? "SIIGO_CONNECTION_FAILED" });
  }
});
