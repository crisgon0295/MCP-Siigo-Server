...
export function createMcpServer() {
  ...
  server.tool("siigo_update_product", "Actualiza propiedades o listas de precio de un producto.", { id: z.string(), product: z.record(z.unknown()) }, ({ id, product }) => run("products.update", (c) => c.request(`/products/${id}`, { method: "PUT", body: JSON.stringify(product) })));
  ...
  server.tool("siigo_get_customer", "Consulta un cliente por ID.", { id: z.string() }, ({ id }) => run("customers.get", (c) => c.request(`/customers/${id}`)));
  ...
  server.tool("siigo_update_customer", "Actualiza un cliente existente.", { id: z.string(), customer: z.record(z.unknown()) }, ({ id, customer }) => run("customers.update", (c) => c.request(`/customers/${id}`, { method: "PUT", body: JSON.stringify(customer) })));
  ...
  server.tool("siigo_get_quotation", "Consulta una cotización por ID.", { id: z.string() }, ({ id }) => run("quotations.get", (c) => c.request(`/quotations/${id}`)));
  ...
  server.tool("siigo_update_quotation", "Actualiza la misma cotización en Siigo.", { id: z.string(), quotation: z.record(z.unknown()) }, ({ id, quotation }) => run("quotations.update", (c) => c.request(`/quotations/${id}`, { method: "PUT", body: JSON.stringify(quotation) })));
