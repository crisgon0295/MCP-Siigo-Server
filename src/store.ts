...
export const store = {
  ...
  configure(input: { companyName: string; siigoUsername: string; accessKey?: string; partnerId: string }) { const state = read(); state.installation = { ...state.installation, companyName: input.companyName, siigoUsername: input.siigoUsername, partnerId: input.partnerId, accessKeyEncrypted: input.accessKey ? encrypt(input.accessKey) : state.installation.accessKeyEncrypted, connectionStatus: "unconfigured", updatedAt: new Date().toISOString() }; write(state); return this.publicInstallation(); },
